...
function Isnow ({datapro}){
     const [cont,setcont]=useState(100)
        let sec= document.getElementById('secnow')
        let i =100
        const handelleft =()=>{
            if(cont<4150){
                setcont(cont+i)  
    ...
    return(
        <>
         ...
         <div className=" w-[1521px] h-[572px] bg-[#FFF7F4] mx-auto flex justify-center flex-col mb-20">
                <div className="w-[300px] h-[50px] text-center bg-white rounded-b-md m-auto mt-[-10px]">
                    <h3 className=" text-[#01070D] text-[32px] ">تخفیفات ویژه</h3>
                </div>
                ...
                <div className="h-full flex justify-evenly m-auto gap-[20px] transition-transform" id="secnow">
                    {  datapro.map((pro)=>{
                        if(pro.isNew == true){
                            return(
                                    <Link key={pro.id} to={`/pro/${pro.id}`}>
                                     <div className="w-[366px] h-[467px] overflow-hidden rounded-[16px]">
